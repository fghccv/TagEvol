        ...
        for i in range(budget):
            ...
            for tags in shot_tags:
                ...
                for key in tags:
                    ...
                    temp_tags += clean_tags
                    merge_tags[clean_key] = list(set(merge_tags[clean_key]))
                temp_tags = list(set(temp_tags))
                all_tags_len.append(len(temp_tags))
            # print(merge_tags)
            avg_len = sum(all_tags_len)/len(all_tags_len)
            merge_tags_len = 0
            for k in merge_tags:
                merge_tags_len += len(merge_tags[k])
            budget_tags_ratio = avg_len / merge_tags_len
