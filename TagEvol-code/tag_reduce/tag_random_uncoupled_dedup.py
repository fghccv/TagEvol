...
def process(instruction):
    ...
    split_words = ["### Python Code:", "### Implementation:", "###Instructions:",\
               "### Solution:", "### Expected SQL Query:",\
               "### JavaScript:"
               ]
    patterns = ["### ?Example.*:", "###.*Implementation:"]
...
class TagReduce:
    ...
    def tags2instruction(self, data_points, temperature, max_tokens, ori_datas, num_shot, budget):
        ...
        progress_bar = tqdm.tqdm(total=budget)
        while len(all_result) < budget:
            assert len(all_result) == len(instructions)
            batch_prompts = []
            for i in range(batch):
                ...
                generate_data_point['shot_tags'] = shot_tags
                generate_data_point['ids'] = shot_ids
                ...
                for k in merge_tags:
                    for v in merge_tags[k]:
                         if random.random() <= budget_tags_ratio:
                            sub_tags[k].append(v)
                generate_data_point['sub_tags'] = sub_tags
                shot_instructions = random.sample(ori_instructions, k=num_shot)
                generate_data_point['shot_instructions'] = shot_instructions
                batch_prompts.append(generate_instruction_prompt(generate_data_point))
                generate_data_points.append(generate_data_point)

            inputs = [[{"role": "user", "content": prompt}] for prompt in batch_prompts]
            ...
            for i in range(len(inputs)):
                ...
                new_instruction_tokens = scorer._tokenizer.tokenize(output.split("###")[0])
                ...
                # print(rouge_scores)
                if rouge_scores!=[] and max(rouge_scores) > 0.7:
                    continue
                all_result.append(
                    {
                        "instruction":output,
                        "input":"",
                        "output":"",
                        "tags": generate_data_points[i]['sub_tags']
                    }
                )
...
def main(model_name_or_path: str, source_file: str,
    ...
    all_results = engine.tags2instruction(data_points, temperature, max_tokens, ori_datas, num_shot, budget)
