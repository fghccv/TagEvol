...
class ResponseGenerator:
    ...
    def generate_responses(self, instructions: List[str], inputs: List[str], outputs: List[str], ids, ori_instructions, tags) -> List[Dict[str, Any]]:
        """
        ...
        return [
            {
                ...
                "ori_instruction": ori_instructions[i],
                "id": ids[i],
                "tags":tags[i]
            }
            for i, (inst, inp) in enumerate(zip(instructions, inputs)) \
            if len(tokenizer.tokenize(resp_outputs[i].outputs[0].text)) < 4094
        ]
...
def main(model_name_or_path: str, inst_file: str, debug: bool) -> None:
    ...
    try:
        if inst_file.endswith('jsonl'):
            with open(inst_file, "r") as f:
                inst_data = [json.loads(l) for l in f]
        else:
            with open(inst_file, "r") as f:
    ...
    responses = response_generator.generate_responses(
        ...
        [item["id"] for item in inst_data],
        [item["ori_instruction"] for item in inst_data],
        [item["tags"] for item in inst_data]
    )
