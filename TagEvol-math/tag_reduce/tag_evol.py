    ...
    def tags_reduce_instruction(self, data_points, temperature, max_tokens, num_pool, evol_use_tag_data_points, num_tag):
        ...
        for id in tqdm.tqdm(data_points):
            ...
            id2inputs[id] = [{"role": "user",
                             "content": generate_evol_prompt_multitag_20mul3up(final_merge_tags, ori_instruction, num_tag)}]
        ...
        while len(use_ids) > 0:
            ...
            for i in range(len(temp_inputs)):
                ...
                try:
                    ...
                    assert "Rewritten Instruction#" in output
                    rewrite_inst = output.split("#Rewritten Instruction#")[1].strip(":").strip()
