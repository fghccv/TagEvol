...
class TagReduce:
    ...
    def generate_tags(self, datas, temperature, max_tokens):
        ...
        idx = 0
        while len(use_ids) > 0:
            sampling_params = SamplingParams(temperature=temperature, max_tokens=max_tokens,seed=42+idx)
    ...
    def tags_reduce(self, data_points, temperature, max_tokens, budget, batch, alpha):
        import re,math
        from collections import defaultdict
        ...
        for i in range(budget):
            ...
            for tags in shot_tags:
                ...
                for key in tags:
                    ...
                    merge_tags[clean_key] = list(OrderedSet(merge_tags[clean_key]))
                temp_lns.append(len_v)
            avg_ln = sum(temp_lns)/len(temp_lns)
            generate_data_points[i]['avg_len'] = avg_ln
            generate_data_points[i]['merge_tags'] = merge_tags
            inputs.append(generate_reduce_prompt(merge_tags))

        tokenizer = AutoTokenizer.from_pretrained(self.model_name_or_path)
        inputs = [tokenizer.apply_chat_template([{"role": "user", "content": it}], tokenize=False, add_generation_prompt=True) for it in inputs]
        use_ids = list(range(budget))
        idx = 0
        while len(use_ids) > 0:
            sampling_params = SamplingParams(temperature=temperature, max_tokens=max_tokens,seed=42+idx)
            temp_inputs = [inputs[i] for i in use_ids]
            temp_outputs = self.llm_engine.generate(temp_inputs, sampling_params)
            temp_ids = []
            for i in range(len(use_ids)):
                output = temp_outputs[i].outputs[0].text.strip()
                try:
                    # print(inputs[use_ids[i]])
                    # print('-------')
                    # print(output)
                    # print("============")
                    # input()
                    assert "#Finally Expanded Tag Dictionary#" in output
                    output = output.split("#Finally Expanded Tag Dictionary#")[-1]
                    tags = re.findall("```(.+?)```", output, re.DOTALL)[0]
                    tags = tags.strip().strip("json").strip()
                    tags = json.loads(tags)
                    # print(generate_data_points[use_ids[i]]['merge_tags'])
                    # input()
                    generate_data_points[use_ids[i]]['final_merge_tags'] = tags
                except Exception as e:
                    temp_ids.append(use_ids[i])
                    continue
            use_ids = temp_ids
        ...
        for i in range(budget):
            ...
            merge_tags = generate_data_points[i]['final_merge_tags']
            merge_tags_ln = 0
            for k in merge_tags:
                merge_tags_ln += len(merge_tags[k])
            budget_tags_ratio = alpha * generate_data_points[i]['avg_len']/merge_tags_ln
            for k in merge_tags:
    ...
    def tags2instruction(self, generate_data_points, temperature, max_tokens, ori_datas, num_shot):
        ori_instructions = [data['instruction'] for data in ori_datas]
        ...
        tokenizer = AutoTokenizer.from_pretrained(self.model_name_or_path)
        sampling_params = SamplingParams(temperature=temperature, max_tokens=max_tokens,seed=42)
        if num_shot != 0:
        ...
        for i in range(len(inputs)):
            ...
            if len(tokenizer.tokenize(outputs[i].outputs[0].text)) >= max_tokens:
                continue
...
def main(model_name_or_path: str, source_file: str,
         ...
         debug: bool, tp: int, batch: int, alpha: float) -> None:
    """
    ...
    generate_data_points = engine.tags_reduce(data_points, temperature, max_tokens, budget, batch, alpha)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--alpha', type=float, default=1, help='Number of tensor parallel workers')

    args = parser.parse_args()

    main(args.model_name_or_path, args.source_file, args.target_file, args.temperature, args.max_tokens, args.budget, args.num_shot,args.debug, args.tp, args.batch, args.alpha)
