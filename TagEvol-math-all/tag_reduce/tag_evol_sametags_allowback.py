...
from prompt import generate_cgtags_prompt, generate_reduce_prompt, generate_evol_allowback_prompt
class TagReduce:
    ...
    def tags_reduce_instruction(self, data_points, temperature, max_tokens, batch, ori_data_points, roud):
        ...
        for idx, dk in tqdm.tqdm(enumerate(ori_data_points)):
            if idx >=100:
                break
            ...
            ori_tags = list(OrderedSet(ori_tags))

            all_instructions.append(generate_evol_allowback_prompt(merge_tags, ori_instruction, ori_tags))
        ...
        all_results = []
        while len(use_ids) > 0:
            temp_inputs = [tokenizer.apply_chat_template(inputs[id], tokenize=False, add_generation_prompt=True) for id in use_ids]
