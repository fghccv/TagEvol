...
from prompt import generate_cgtags_prompt, generate_reduce_prompt, generate_evol_random_prompt
...
class TagReduce:
    ...
    def tags_reduce_instruction(self, data_points, temperature, max_tokens, batch):
        import re,math
        from collections import defaultdict
        generate_data_points = defaultdict(dict)
        all_instructions = []
        # idx = 0
        for dk in tqdm.tqdm(data_points):
            # idx += 1
            # if idx >=100:
            #     break
            ...
            select_tags = random.sample(merge_tags, k=1)
            # print(ori_tags)
            # print(merge_tags)
            # input()
            all_instructions.append(generate_evol_random_prompt(select_tags, ori_instruction))
...
def main(model_name_or_path: str, source_file: str,
    ...
    all_results = engine.tags_reduce_instruction(data_points, temperature, max_tokens, batch)
...
if __name__ == "__main__":
    ...
    main(args.model_name_or_path, args.source_file, args.target_file, args.temperature, args.max_tokens, args.debug, args.tp, args.batch)
