...
class TagReduce:
    ...
    def generate_tags(self, datas, temperature, max_tokens):
        ...
        id2input = {id:tag_input for id, tag_input in zip(ids, tag_inputs)}
        id2instructions = {id:inst for id, inst in zip(ids, instructions)}
    ...
    def tags_reduce_instruction(self, data_points, temperature, max_tokens, batch, ori_data_points, roud):
        import re,math
        from collections import defaultdict

        generate_data_points = defaultdict(dict)
        all_instructions = []
        all_ids = list(data_points.keys())
        for _ in range(roud):
            random.shuffle(all_ids)
        all_oridatas_ids = list(ori_data_points.keys())
        for m,idx in tqdm.tqdm(enumerate(all_oridatas_ids)):
            dk = str(idx)
            shot_ids = random.sample(all_ids, k=batch)
            ...
            ori_instruction = ori_data_points[dk]['instruction']
            ori_tags = []
            for key in ori_data_points[dk]['tags']:
                clean_tags = [str(v).lower().strip().replace('_', ' ') for v in ori_data_points[dk]['tags'][key]]
                ori_tags += clean_tags
        ...
        tokenizer = AutoTokenizer.from_pretrained(self.model_name_or_path)
        sampling_params = SamplingParams(temperature=temperature, max_tokens=max_tokens)
        id2inputs = {id:[{"role": "user", "content": inst}] for id,inst in zip(all_oridatas_ids, all_instructions)}
        use_ids = all_oridatas_ids
        all_results = []
        while len(use_ids) > 0:
...
def main(model_name_or_path: str, source_file: str,
         ...
         debug: bool, tp: int, batch: int, tags_file: str, roud: int) -> None:
    """
    ...
    tags_datas = get_ori_datas(tags_file)
    if debug:
    ...
    if os.path.exists(f"{tags_file}.cgtag_datas.json"):
        print(f"{tags_file}.cgtag_datas.json exsits")
        data_points = json.load(open(f"{tags_file}.cgtag_datas.json"))
    else:
        print(f"{tags_file}.cgtag_datas.json not exsits")
        data_points = engine.generate_tags(tags_datas, temperature, max_tokens)
        json.dump(data_points, open(f"{tags_file}.cgtag_datas.json", 'w'))
    if os.path.exists(f"{source_file}.cgtag_datas.json"):
        print(f"{source_file}.cgtag_datas.json exsits")
        ori_data_points = json.load(open(f"{source_file}.cgtag_datas.json"))
    else:
        print(f"{source_file}.cgtag_datas.json not exsits")
        ori_data_points = engine.generate_tags(ori_datas, temperature, max_tokens)
        json.dump(ori_data_points, open(f"{source_file}.cgtag_datas.json", 'w'))
    for data in ori_datas:
        assert data['instruction'] == ori_data_points[str(data['id'])]['instruction']
    all_results = engine.tags_reduce_instruction(data_points, temperature, max_tokens, batch, ori_data_points, roud)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tags_file', type=str, required=True, help='Path to the source JSON file')
    ...
    parser.add_argument('--roud', type=int, default=8, help='Number of tensor parallel workers')

    args = parser.parse_args()

    main(args.model_name_or_path, args.source_file, args.target_file, args.temperature, args.max_tokens, args.debug, args.tp, args.batch, args.tags_file, args.roud)
