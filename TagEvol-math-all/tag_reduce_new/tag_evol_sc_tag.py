...
class TagReduce:
    ...
    def generate_tags(self, datas, temperature, max_tokens):
        ...
        id2input = {data['id']: \
            [{"role": "user", "content": generate_cgtags_prompt(data['instruction'])}] \
             for data in datas}
        use_ids = [data['id'] for data in datas]
        id2datapoints = defaultdict(list)
        while len(use_ids) > 0:
            ...
            for i in range(len(tag_inputs)):
                for j in range(N):
                    if len(id2datapoints[use_ids[i]]) >= num_sample:
                        continue
                    output = tag_outputs[i].outputs[j].text.strip()
                    try:
                        tags = re.findall("```(.+?)```", output, re.DOTALL)[0]
                        tags = tags.strip().strip("json").strip()
                        tags = json.loads(tags)
                        id2datapoints[use_ids[i]].append(tags)
                    except Exception as e:
                        pass
                if len(id2datapoints[use_ids[i]]) < num_sample:
                    temp_ids.add(use_ids[i])
            use_ids = list(temp_ids)
        id2datapoints_final = {}
        for id in id2datapoints:
            key_counter = []
            for tags in id2datapoints[id]:
                for k in tags:
                    for v in tags[k]:
                        key_counter.append(str(v).lower().strip().replace('_', ' '))
            key_counter = Counter(key_counter)
            final_tags = [key for key in key_counter if key_counter[key]>=2]
            id2datapoints_final[id] = {
                "instruction": id2instructions[id],
                "tags":final_tags
            }



        return id2datapoints_final

    def tags_reduce_instruction(self, data_points, temperature, max_tokens, batch, evol_use_tag_data_points, round):
        import re,math
        from collections import defaultdict
...
def main(model_name_or_path: str, source_file: str,
         ...
         debug: bool, tp: int, batch: int, tag_file:str, round: int) -> None:
    """
    ...
    #加载当前待进化文本的tag文件
    if os.path.exists(f"{source_file}.sctag_datas.json"):
        print(f"{source_file}.sctag_datas.json exsits")
        data_points = json.load(open(f"{source_file}.sctag_datas.json"))
    else:
        print(f"{source_file}.sctag_datas.json not exsits")
        data_points = engine.generate_tags(ori_datas, temperature, max_tokens)
        json.dump(data_points, open(f"{source_file}.sctag_datas.json", 'w'))
    #加载待进化使用的tag文件
    if os.path.exists(f"{tag_file}.sctag_datas.json"):
        print(f"{tag_file}.sctag_datas.json exsits")
        evol_use_tag_data_points = json.load(open(f"{tag_file}.sctag_datas.json"))
    else:
        print(f"{tag_file}.sctag_datas.json not exsits")
        evol_use_tag_data_points = engine.generate_tags(evol_tags_datas, temperature, max_tokens)
        json.dump(evol_use_tag_data_points, open(f"{tag_file}.sctag_datas.json", 'w'))
    all_results = engine.tags_reduce_instruction(data_points, temperature, max_tokens, batch, evol_use_tag_data_points, round)
...
if __name__ == "__main__":
    ...
    main(args.model_name_or_path, args.source_file, args.target_file, args.temperature, args.max_tokens, args.debug, args.tp, args.batch, args.tag_file, args.round)
