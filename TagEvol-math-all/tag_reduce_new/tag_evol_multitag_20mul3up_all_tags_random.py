...
from prompt import generate_cgtags_prompt, generate_reduce_prompt, generate_evol_prompt_multitag_20mul3up_random
...
class TagReduce:
    ...
    def tags_reduce_instruction(self, data_points, temperature, max_tokens, evol_use_tag_data_points, round, num_tag):
        import re,math
        from collections import defaultdict
        ...
        for id in tqdm.tqdm(data_points):
            # if int(id)>100:
            #     break
            merge_tags = random.sample(all_evol_use_tags, k=num_tag)
            ...
            id2inputs[id] = [{"role": "user",
                             "content": generate_evol_prompt_multitag_20mul3up_random(final_merge_tags, ori_instruction, num_tag)}]
        ...
        while len(use_ids) > 0:
            ...
            for i in range(len(temp_inputs)):
                ...
                try:
                    ...
                    rewrite_inst = output.split("Finally Rewritten Instruction")[1].strip(":").strip("#").strip()
...
def main(model_name_or_path: str, source_file: str,
         ...
         debug: bool, tp: int, tag_file:str, round: int, num_tag: int) -> None:
    """
    ...
    all_results = engine.tags_reduce_instruction(data_points, temperature, max_tokens, evol_use_tag_data_points, round, num_tag)
...
if __name__ == "__main__":
    ...
    main(args.model_name_or_path, args.source_file, args.target_file, args.temperature, args.max_tokens, args.debug, args.tp, args.tag_file, args.round, args.num_tag)
