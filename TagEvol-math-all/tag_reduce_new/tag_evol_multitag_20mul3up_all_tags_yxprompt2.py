...
from prompt import generate_cgtags_prompt, generate_reduce_prompt, generate_evol_prompt_multitag_20mul3up_yxprompt2
...
class TagReduce:
    ...
    def tags_reduce_instruction(self, data_points, temperature, max_tokens, num_pool, evol_use_tag_data_points, num_tag):
        import re,math
        from collections import defaultdict
        ...
        for id in tqdm.tqdm(data_points):
            ...
            id2inputs[id] = [{"role": "user",
                             "content": generate_evol_prompt_multitag_20mul3up_yxprompt2(final_merge_tags, ori_instruction, list(OrderedSet(ori_tags)), num_tag)}]
        ...
        while len(use_ids) > 0:
            ...
            for i in range(len(temp_inputs)):
                ...
                try:
                    ...
                    rewrite_inst = output.split("<begin>")[1].strip()
                    rewrite_inst = rewrite_inst.split("<end>")[0].strip()
                    assert len(rewrite_inst) > 0
                    tag_datas = output
                    # print(temp_inputs[i])
                    # print("-------")
                    # print(output)
                    # print(data_points[use_ids[i]]['instruction'])
                    # print(rewrite_inst)
                    # input()
                    all_results.append(
                        {
...
def main(model_name_or_path: str, source_file: str,
    ...
    all_results = engine.tags_reduce_instruction(data_points, temperature, max_tokens, num_pool, evol_use_tag_data_points, num_tag)
