...
def construct_prompt(dataset,tokenizer=None):
    ...
    if dataset == "math" or dataset == "math-500":
        prompts = [problems[task_id]['instruction'].strip() + "\nPlease reason step by step, and put your final answer within \\boxed{}." for task_id in problems]
    else:
...
def main():
    ...
    llm = LLM(model=args.model, tensor_parallel_size=4, max_model_len=8192)
    stop = ["</s>", "\n\n\n\n", "<|EOT|>"]
